...
ALIST_EXE = WORKDIR.joinpath("alist/alist")
...
assert subprocess.run([ALIST_EXE, 'start']).returncode == 0, "Alist 启动失败"
time.sleep(5)
client = Client("http://localhost:5244", username='admin', password='123456')

test_dir = WORKDIR.joinpath("test_dir")
...
local_storage = {
    ...
    "cache_expiration": 0,
    "status": "work",
    "addition": "{\"root_folder_path\":\"%s\",\"thumbnail\":false,\"thumb_cache_folder\":\"\",\"show_hidden\":true,\"mkdir_perm\":\"750\"}" % test_dir.absolute(),
    "remark": "",
